...
const createBill = async (req, res) => {
  try {
    ...
    if (!order.status === "Completed") {
      return res
        .status(404)
        .json({ error: "Bill is generted after completeing the service" });
    }
    const basicAmount = service.price;
    const totalPrice =
      Number(basicAmount) + Number(materialCost) + Number(extraCharges);
...
const getBillByOrder = async (req, res) => {
  try {
    ...
    if (!bill) {
      return res
        .status(404)
        .json({ success: false, message: "Bill not found for this order" });
    }
...
const getAdminPayments = async (req, res) => {
  try {
    const bills = await billModel
      .find()
      .populate("customer_id provider_id order_id");
    console.log(bills);
...
const getProviderPayments = async (req, res) => {
  try {
    ...
    const bills = await billModel
      ...
      .populate("customer_id order_id provider_id");
