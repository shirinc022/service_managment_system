  ...
  try {
    ...
    if (
      ...
      return res
        ...
        .json({
          error:
            "All customer details (name, phone, address, location) are required",
        });
    }
    ...
    if (!customerId) {
      return res
        .status(400)
        .json({ error: "Customer should login before placing order" });
    }
    ...
    if (!providerId) {
      return res
        .status(400)
        .json({ error: "Provider ID is missing in service" });
    }
    ...
    if (existingOrder) {
      return res
        .status(400)
        .json({ error: "You already have a pending request for this service" });
    }
...
const getOrders = async (req, res) => {
  try {
    ...
    if (!orders.length) {
      return res
        .status(404)
        .json({ message: "No orders found for this provider" });
    }
...
const customerOrderView = async (req, res) => {
  try {
    ...
    if (!customerId) {
      return res
        .status(401)
        .json({ error: "Unauthorized: Please log in to view your requests" });
    }
    const orders = await orderModel
...
const deleteOrder = async (req, res) => {
  try {
    ...
    if (!customerId) {
      return res
        .status(401)
        .json({ error: "Unauthorized: Please log in to delete orders" });
    }
...
const updateBillStatus = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Bill status updated", order: updatedOrder });
  } catch (error) {
    console.log(error);
    res
      .status(error.status || 500)
