...
export default function ProviderTable() {
  ...
  const handleVerify = (providerId) => {
    adminVerifyProvider(providerId)
      .then((res) => {
        console.log(res);
  ...
  const handleReject = (providerId) => {
    adminRejectProvider(providerId)
      .then((res) => {
        console.log(res);
        window.location.reload();
      })
            ...
            <tbody>
              {providers.map((provider, index) => (
                ...
                >
                  ...
                  <td className="p-4 text-sm text-gray-500 break-words max-w-[150px]">
                    {provider.email}
                  </td>
                  <td className="p-4 text-sm text-blue-500 cursor-pointer hover:underline">
                    <a
                      href={provider.document}
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      <FaFilePdf className="inline-block w-5 h-5 text-red-500" />
                      Document
                    </a>
                  </td>
                  <td>
                    <button
                      className={`p-4 font-bold badge  ${
